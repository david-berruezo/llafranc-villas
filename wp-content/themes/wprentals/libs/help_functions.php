...
function wpestate_generate_trip_details($property_id,$invoice_id,$bookid,$mode=''){
    ...
    print '<div class="trip_details_wrapper"><div class="trip_details_container" style="'.esc_attr($mode_wrapper_trip_details_container).'" >';
        ...
        print '<h5 style="'.esc_attr($mode_wrapper_h5).'" >'.esc_html('Dates','wprentals').'</h5>';

        print wpestate_show_labels('no_of_nights', $rental_type, $booking_type) ;
        print ': '.$booking_array['numberDays'];
        
        print '<br>';
        print '<strong>'.esc_html('from ','wprentals').'</strong>'.$from_weekday.', '.wpestate_convert_dateformat_reverse($booking_from_date).' <strong>'.esc_html__( 'to','wprentals').'</strong> '.$to_weekday.', '.wpestate_convert_dateformat_reverse($booking_to_date);
       
        
        if($booking_guests>0){
            print '<h5 style="'.esc_attr($mode_wrapper_h5).'" >'.esc_html('Guests','wprentals').'</h5>';
        ...
        print '<h5 style="'.esc_attr($mode_wrapper_h5).'" >'.esc_html('Address','wprentals').'</h5>';
        ...
        print '<h5 style="'.esc_attr($mode_wrapper_h5).'" >'.esc_html('Your Host','wprentals').'</h5>';
        ...
        print '<h5 style="'.esc_attr($mode_wrapper_h5).'" >'.$owner_details['name'].'</h5>';
            
        if($owner_details['agent_phone']!=''){
            print '<strong>'.esc_html__('Phone','wprentals').':</strong> '.$owner_details['agent_phone'].'<br>';
        }
        if($owner_details['agent_mobile']!=''){
            print '<strong>'.esc_html__('Mobile','wprentals').':</strong> '.$owner_details['agent_mobile'].'<br>';
        }
        if($owner_details['agent_email']!=''){
            print '<strong>'.esc_html__('Email','wprentals').':</strong> '.$owner_details['agent_email'].'<br>';
        }
        if($owner_details['agent_skype']!=''){
            print '<strong>'.esc_html__('Skype','wprentals').':</strong> '.$owner_details['agent_skype'].'<br>';
        }
         
        $do_we_show         =   trim(wprentals_get_option('wp_estate_show_terms_conditions', ''));
        
        if($do_we_show=='yes'):
            print '<h5 style="'.esc_attr($mode_wrapper_h5).'" >'.esc_html(' House Rules','wprentals').'</h5>';

            $cancellation_policy   = esc_html(get_post_meta($property_id, 'cancellation_policy', true));
            if(trim($cancellation_policy)!=''){
                print '<h6 style="'.esc_attr($mode_wrapper_h6).'" >'.esc_html('Cancellation Policy','wprentals').'</h6>';
                print esc_html($cancellation_policy).'</br>';
            }

            $smoking_allowed   = esc_html(get_post_meta($property_id, 'smoking_allowed', true));
            if($smoking_allowed!=''){
                print '<h6 style="'.esc_attr($mode_wrapper_h6).'" >'.esc_html('Smoking Allowed','wprentals').'</h6>';
                print esc_html($smoking_allowed).'</br>';
            }

            $pets_allowed  = esc_html(get_post_meta($property_id, 'pets_allowed', true));
            if($pets_allowed!=''){
                print '<h6 style="'.esc_attr($mode_wrapper_h6).'" >'.esc_html('Pets Allowed','wprentals').'</h6>';
                print esc_html($pets_allowed).'</br>';
            }

            $party_allowed  = esc_html(get_post_meta($property_id, 'party_allowed', true));
            if($party_allowed!=''){
                print '<h6 style="'.esc_attr($mode_wrapper_h6).'" >'.esc_html('Party Allowed','wprentals').'</h6>';
                print esc_html($party_allowed).'</br>';
            }

            $children_allowed   = esc_html(get_post_meta($property_id, 'children_allowed', true));
            if($children_allowed!=''){
                print '<h6 style="'.esc_attr($mode_wrapper_h6).'" >'.esc_html('Children Allowed','wprentals').'</h6>';
            ...
            if(trim($other_rules)!=''){
                print '<h6 style="'.esc_attr($mode_wrapper_h6).'" >'.esc_html('Other Rules','wprentals').'</h6>';
...
if (!function_exists('wpestate_share_unit_desing')):
    ...
    function wpestate_share_unit_desing($prop_id, $is_single = 1) {
        ...
        <div class="prop_social">
            ...
            <a href="https://twitter.com/home?status=<?php echo urlencode($title . ' ' . esc_url($link)); ?>" class="share_tweet" target="_blank" rel ="noreferrer" ><i class="fab fa-twitter"></i></a>
