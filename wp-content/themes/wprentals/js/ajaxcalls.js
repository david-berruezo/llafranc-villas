...
function wpestate_start_filtering_new(newpage) {
    ...
    let services = [];
    let features = [];
    let atributos = jQuery(".buscador_amenities input");
    ...
    for (let index = 0; index < atributos.length; index++) {
        ...
        if (jQuery(atributo).is(":checked")){
            ...
            if (encontrado != -1){
                ...
                let codigo_service = atributo.replace("services_","");
                services.push(codigo_service);
            }
        }// end if

    }

    let services_string = services.join(",");
    let features_string = features.join(",");

    print_values_wpestate_start_filtering(services,features,services_string,features_string);
...
function wpestate_start_filtering(newpage) {
    ...
    console.log("objeto"+services_selector);
    console.log("titulo:"+services_selector.text);
    console.log("value: "+jQuery(".mi_selector").find(':selected').data('value'));
    console.log("frita: "+jQuery('.mi_selector').find(':selected'));


    jQuery('#grid_view').addClass('icon_selected');
    jQuery('#list_view').removeClass('icon_selected');
    var action, category, city, area, order, ajaxurl, page_id , services;
    // get action vars
    action = jQuery('#a_filter_action').attr('data-value');
    // get category
    category = jQuery('#a_filter_categ').attr('data-value');
    ...
    area = jQuery('#a_filter_areas').attr('data-value');
    // get servcios
    //services = jQuery('#a_filter_extra_services').attr('data-value');
...
//end delete profile
    ...
    // select2
    $('.mi-selector').on('select2:closing', function (e) {
        //print_message("closing");
    });

    $('.mi-selector').on('select2:close', function (e) {
        //print_message("close");
    });

    $('.mi-selector').on('select2:opening', function (e) {
        //print_message("opening");
    });

    $('.mi-selector').on('select2:open', function (e) {
        //print_message("open");
    });

    $('.mi-selector').on('select2:selecting', function (e) {
        //print_message("selecting");
    });

    $('.mi-selector').on('select2:select', function (e) {
        //print_message("select");
        //recoger_parametros(e);
        //print_message("---------- final -----------");
    });

    $('.mi-selector').on('select2:unselecting', function (e) {
        recoger_parametros(e);
        //print_message("unselecting");
    });

    $('.mi-selector').on('select2:unselect', function (e) {
        //print_message("unselect");
    });

    $('.mi-selector').on('select2:clearing', function (e) {
        //print_message("clearing");
    });

    $('.mi-selector').on('select2:clear', function (e) {
        //print_message("clear");
    });
    ...
    let print_message = mensaje => {
        console.log(mensaje);
    }


    let recoger_parametros = e => {
        let data = e.params.data;
        console.log("objeto"+data);
        console.log("titulo:"+data.text);
        console.log("id:"+data.id);
        console.log("value:"+data.value);

        console.log("value: "+$(this).find(':selected').data('value'));
        console.log("total: "+$(".mi-selector").select2("data"));
        let total = $(".mi-selector").select2("data");
        for (let i=0; i<total.length;i++){
            console.log("objeto select"+total[i]);
        }
    }
