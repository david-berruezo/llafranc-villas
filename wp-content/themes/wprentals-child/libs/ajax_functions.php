...
if (!function_exists('wpestate_ajax_filter_listings')):
    ...
    function wpestate_ajax_filter_listings()
    {
        ...
        $vector_feature = $vector_feature_code = $vector_feature_title = array();
        $vector_service = $vector_service_code = $vector_service_title = array();

        //////////////////////////////////////////////////////////////////////////////////////
        ///// feature filters
        //////////////////////////////////////////////////////////////////////////////////////
        if(isset($_POST['feature']) && $_POST['feature'] != ""){
            $vector_feature_code = explode(",", $_POST['feature']);
            if (count($vector_feature_code) > 0){
                for ($i=0;$i<count($vector_feature_code);$i++ ) {
                    $title = getTextosFeature($actual_language);
                    $elemento = $title[$vector_feature_code[$i]];
                    array_push($vector_feature_title , $elemento);
                }
                $vector_feature =  (count($vector_feature_title) > 1 ) ? implode(",",$vector_feature_title) : $vector_feature_title[0];
            }else{
                $title = getTextosFeature($actual_language);
                $elemento = $title[$vector_feature_code[$vector_feature_code]];
                array_push($vector_feature_title , $title[$elemento]);
                $vector_feature =  (count($vector_feature_title) > 1 ) ? implode(",",$vector_feature_title) : $vector_feature_title[0];
            }
        ...
        if($vector_feature){
            $taxarea[] = sanitize_title(wp_kses($vector_feature, $allowed_html));
            $feature_array = array(
        ...
        if(isset($_POST['service']) && $_POST['service'] != ""){
            $vector_service_code = explode(",",$_POST['service']);
            if (count($vector_service_code) > 0){
                for ($i=0;$i<count($vector_service_code);$i++ ) {
                    $title = get_service_name_by_code($vector_service_code[$i] , $actual_language);
                    $elemento = $title->text_title;
                    array_push($vector_service_title,$elemento);
                }// end for
                $vector_service =  (count($vector_service_title) > 1 ) ? implode(",",$vector_service_title) : $vector_service_title[0];
            }else{
                $title = get_service_name_by_code($vector_service_code , $actual_language);
                $elemento = $title->text_title;
                array_push($vector_service_title , $elemento);
                $vector_service =  (count($vector_service_title) > 1 ) ? implode(",",$vector_service_title) : $vector_service_title[0];
            }// end if
        ...
        if($vector_service){
            $taxarea[] = sanitize_title(wp_kses($vector_service, $allowed_html));
            $service_array = array(
