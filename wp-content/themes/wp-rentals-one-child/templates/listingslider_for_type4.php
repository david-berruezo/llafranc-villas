...
<div class="listing_main_image header_masonry panel-body imagebody imagebody_new" id="">
        ...
        # 371950
        if ($_SERVER["REMOTE_ADDR"] == "79.153.38.91"){
            print_r($my_post);
        }
        ...
        if(pll_current_language() == "es"){
            ...
            if ($_SERVER["REMOTE_ADDR"] == "79.153.38.91"){
                ...
                if ($my_post == "371950"){
                    $sql = "select * from wp_posts where post_type = 'attachment' and post_parent = '".$my_post."' order by ID ";
                    $post_attachments   = $wpdb->get_results($sql);
                }else{
                    $arguments      =   array(
                        'numberposts'   =>  -1,
		...
		if(pll_current_language() == "es"){
            ...
            if ($_SERVER["REMOTE_ADDR"] == "79.153.38.91"){
                ...
                switch ($my_post){
                    case "371950201": $my_post = 371950;
                    break;
                    case "371950101": $my_post = 371950;
                    break;
                    case "371950401": $my_post = 371950;
                    break;
                }

                $sql = "select * from wp_posts
where post_type = 'attachment' and post_parent = '".$my_post."' order by ID ";
                $post_attachments   = $wpdb->get_results($sql);
